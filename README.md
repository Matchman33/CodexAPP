...
AI 在当前项目目录内生成图片、Excel、Word、PDF、压缩包等文件后，网页聊天会显示附件条目，包含文件名、大小和下载按钮。PNG、JPEG、WebP、GIF 可点击眼睛图标预览原图；Excel 等文档先下载，再用手机或电脑上的相应软件打开，本次不提供表格在线编辑或文档内嵌预览。
...
- NPS 转发下，手机继续使用原 NPS 访问地址，附件沿同一条聊天连接传输，不访问电脑的 `127.0.0.1` 或磁盘路径。聊天中的 Windows 绝对路径会映射为附件操作；文字文件链接下载，图片引用和眼睛按钮预览。接收完成后保留「保存已接收文件」按钮，浏览器未自动保存时可手动点击。
